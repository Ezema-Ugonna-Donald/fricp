                ...
                <?php endforeach; ?>
                    ...
                    <div class="pagination-container">
                        <ul class="pagination-ul">
                            ...
                            <li>
                                <a href="<?= URLROOT ?>/genres/releases/<?= $data ['releaseKey'] ?>/?page=<?= $data ['pageNum'] - 1 ?>">&laquo;</a>
                            </li>
                            <?php endif; ?>

                            <?php for ($i = 1; $i <= $data ['numGenres']; $i++) : ?>
                            <li class="<?= ($i == $data ['pageNum']) ? 'active' : '' ?>">
                                <a href="<?= URLROOT ?>/genres/releases/<?= $data ['releaseKey'] ?>/?page=<?= $i ?>"><?= $i ?></a>
                            </li>
                            <?php endfor; ?>

                            <?php if ($data ['pageNum'] + 1 <= $data ['numGenres']) : ?>
                            <li>
                                <a href="<?= URLROOT ?>/genres/releases/<?= $data ['releaseKey'] ?>/?page=<?= $data ['pageNum'] + 1 ?>">&raquo;</a>
                            </li>
                            <?php endif; ?>
                        </ul>
                    </div>
